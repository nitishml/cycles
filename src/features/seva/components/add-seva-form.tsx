...
import { addSevaFormSchema } from "../types"
import { useAddSeva } from "../hooks/use-add-seva"

type Props = {
    staffRole: "STAFF" | "DIRECTOR";
}
export function AddSevaForm({ staffRole }: Props) {
    const redirectRouter = staffRole === "STAFF" ? "staff" : "director"

    const mutation = useAddSeva()
    const [isLoading, setLoading] = useState(false)
    const router = useRouter()

    const form = useForm<z.infer<typeof addSevaFormSchema>>({
        resolver: zodResolver(addSevaFormSchema) as any,
        defaultValues: {

        }
    })

    function onSubmit(values: z.infer<typeof addSevaFormSchema>) {
        ...
        mutation.mutate({
            ...
            onSuccess: (data) => {
                if (data.success && data.data) {
                    ...
                    router.push(`/${redirectRouter}/seva/manage/${data.data.id}`)
                }
                else {
    ...
    return (
        <div className="w-full flex-1 flex flex-col gap-8 items-start justify-center">
            <InPageHeader label="Add Seva Form" />
