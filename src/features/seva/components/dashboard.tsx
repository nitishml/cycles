"use client"

import { DataError, QueryLoading } from "@/components/query-loaders";
import { useGetSevaList } from "../hooks/use-get-sevas";
import { SevaListTable } from "./seva-list-table";
import { Button } from "@/components/ui/button";
import { PlusCircle } from "lucide-react";
import Link from "next/link";

type Props = {
    staffRole: "STAFF" | "DIRECTOR";
}
export function SevaDashboard({ staffRole }: Props) {
    const redirectRouter = staffRole === "STAFF" ? "staff" : "director"
    const query = useGetSevaList()

    const isDisabled = query.isLoading || query.isPending || query.isFetching
    if (isDisabled) return <QueryLoading />

    if (!query.data || !query.data.data) return <DataError />
    const data = query.data.data

    return (

        <div className="w-full space-y-4">
            {/* <TeacherStatistics data={data.teachers} /> */}
            <Button className="" size={'lg'} asChild>
                <Link href="/staff/seva/create">
                    <PlusCircle /> Create New
                </Link>
            </Button>
            <SevaListTable seva={data.seva} staffRole={staffRole} />
        </div>


    );
}