    ...
    <TimePickerInputGroupContext.Provider value={inputGroupContextValue}>
      <PopoverAnchor asChild>
        <InputGroupPrimitive
          ...
          {...inputGroupProps}
          className={cn(
            "flex min-h-10 w-full cursor-text items-center gap-0.5 rounded-md border border-input bg-background px-3 py-2 shadow-xs outline-none transition-shadow",
...
function TimePickerTrigger(props: ButtonProps) {
  ...
  return (
    <PopoverTrigger
      ...
      {...triggerProps}
      className={cn(
        "cursor-pointer p-2 flex h-10 w-20 justify-center items-center text-foreground transition-colors bg-foreground rounded-md disabled:pointer-events-none [&>svg:not([class*='size-'])]:size-4 [&>svg:not([class*='size-'])]:stroke-background",
        className,
      )}
    >
      {children ?? <Clock />}
  ...
  return (
    <TimePickerGroupContext.Provider value={groupContextValue}>
      <PopoverContent
        ...
        {...contentProps}
        className={cn(
          "flex max-w-(--radix-popover-trigger-width) p-0",
          className,
        )}
...
function TimePickerColumnItem(props: TimePickerColumnItemProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "w-full rounded px-3 py-3 text-left text-sm hover:bg-accent hover:text-accent-foreground focus:border-ring focus:outline-none focus:ring-[3px] focus:ring-ring/50",
        "data-selected:bg-primary data-selected:text-primary-foreground data-selected:hover:bg-primary data-selected:hover:text-primary-foreground text-center",
        className,
      )}
      onClick={onClick}
      onKeyDown={onKeyDown}
...
function TimePickerHour(props: TimePickerHourProps) {
  ...
  return (
    <HourPrimitive
      ...
      {...hourProps}
      className={cn(
        "scrollbar-none flex max-h-[200px] flex-col gap-1 overflow-y-auto p-1 w-[100px]",
        className,
      )}
    >
      {hours.map((hour) => (
...
function TimePickerPeriod(props: DivProps) {
  ...
  return (
    <PeriodPrimitive
      ...
      {...periodProps}
      className={cn("flex flex-col gap-1 p-1 w-[50px]", className)}
    >
      {PERIODS.map((period) => (
        <TimePickerColumnItem
          key={period}
